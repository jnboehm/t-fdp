...
import numpy as np
import pyfftw
from pyfftw.interfaces.numpy_fft import irfft2, rfft2
...
def ibFFT_repulsive(
    ...
    n_boxes_per_dim = int(
        np.minimum(
            np.sqrt(16 * N),
            np.maximum(
                np.sqrt(4 * N / np.log(N)),
                np.maximum(
                    min_num_intervals,
                    (max_coord.item() - min_coord.item())
                    / intervals_per_integer,
                ),
            ),
        )
    )
    allowed_n_boxes_per_dim = np.array(
        [
            50,
            54,
            64,
    ...
    if n_boxes_per_dim < allowed_n_boxes_per_dim[-1]:
        n_boxes_per_dim = allowed_n_boxes_per_dim[
            (allowed_n_boxes_per_dim > n_boxes_per_dim)
        ][0]
    else:
    ...
    if int(gamma) == gamma:
        half_kernel = paraFactor / (
            (
                1.0
                + (
                    np.arange(n_interpolation_points_1d) ** 2
                    + (np.arange(n_interpolation_points_1d) ** 2).reshape(
                        -1, 1
                    )
                )
                * whsquare
        ...
        half_kernel = paraFactor * np.power(
            1.0
            + (
                np.arange(n_interpolation_points_1d) ** 2
    ...
    circulant_kernel_tilde[
        n_interpolation_points_1d:, n_interpolation_points_1d:
    ] = half_kernel
    circulant_kernel_tilde[
        1 : n_interpolation_points_1d + 1, n_interpolation_points_1d:
    ] = np.flipud(half_kernel)
    circulant_kernel_tilde[
        n_interpolation_points_1d:, 1 : n_interpolation_points_1d + 1
    ] = np.fliplr(half_kernel)
    circulant_kernel_tilde[
        1 : n_interpolation_points_1d + 1, 1 : n_interpolation_points_1d + 1
    ] = np.fliplr(np.flipud(half_kernel))
    ...
    w_coefficients = np.zeros(
        (
            n_boxes_per_dim * n_interpolation_points,
            n_boxes_per_dim * n_interpolation_points,
            squared_n_terms,
        ),
        dtype=np.float32,
    ...
    mat_w = np.zeros(
        (
            2 * n_boxes_per_dim * n_interpolation_points,
            2 * n_boxes_per_dim * n_interpolation_points,
            n_terms,
        ),
        dtype=np.float32,
