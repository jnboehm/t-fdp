...
block_dim = (512,)
cudarawtext = open(
    os.path.join(os.path.dirname(__file__), "RawCudafloat.cu")
).read()
...
def ibFFT_repulsive(
    ...
    y_tilde_spacings = cnp.array(
        cnp.arange(n_interpolation_points) * h + h / 2, dtype=cnp.float32
    )
    circulant_kernel_tilde = cnp.zeros(
        (n_fft_coeffs, n_fft_coeffs), dtype=cnp.float32
    )
    Circulant_kernel_tilde_cu(
        grid_dim,
        block_dim,
        (
    ...
    Box_idx_cu(
        grid_dim,
        block_dim,
        (box_idx, Y, box_width, min_coord, n_boxes_per_dim, N),
    )
    y_in_box = cnp.zeros_like(Y)
    Y_in_box_cu(
        grid_dim,
        block_dim,
        (y_in_box, Y, box_idx, box_width, min_coord, n_boxes_per_dim, N),
    )
